...
def edit_category(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_category_name, category_id)
...
➖➖➖➖➖➖➖➖➖➖
    ...
    if categories:
        ...
        for category in categories:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text=category[1], callback_data=f"categoryeditproduct_{category[0]}"))
...
def view_category_products_edit(call):
    ...
    cur.execute(
        "SELECT id, name_tovar, price, kolvo, file_id, file_name, type FROM market WHERE category_id=%s", category_id)
...
➖➖➖➖➖➖➖➖➖➖
    ...
    if products:
        ...
        for product in products:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text=product[1], callback_data=f"productedit_{product[0]}_{category_id}"))

        keyboard.add(telebot.types.InlineKeyboardButton(
            text="Назад", callback_data="back_to_categories_edit_product"))

        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text=messagetext, reply_markup=keyboard, parse_mode="HTML")
    else:

        bot.answer_callback_query(
            callback_query_id=call.id, text="К сожалению, в этой категории нет товаров.")
...
📃 Описание: <b>{product[6]}</b>
        ...
        keyboard = telebot.types.InlineKeyboardMarkup()
        keyboarditem1 = (telebot.types.InlineKeyboardButton(
            text="🏷 Изменить название ", callback_data=f"edit_name_{product_id}"))
        keyboarditem2 = (telebot.types.InlineKeyboardButton(
            text="💰 Изменить цену ", callback_data=f"edit_price_{product_id}"))
        keyboarditem3 = (telebot.types.InlineKeyboardButton(
            text="📦 Изменить количество ", callback_data=f"edit_quantity_{product_id}"))
        keyboarditem4 = (telebot.types.InlineKeyboardButton(
            text="📃 Изменить описание ", callback_data=f"edit_opisanie_{product_id}"))
        keyboarditem5 = (telebot.types.InlineKeyboardButton(
            text="🗂 Изменить файл ", callback_data=f"edit_file_{product_id}"))
        keyboarditem6 = (telebot.types.InlineKeyboardButton(
            text="🖼 Изменить фото ", callback_data=f"edit_photo_{product_id}"))
        ...
        bot.send_photo(
            chat_id,  product[7], caption=message, reply_markup=keyboard, parse_mode="HTML")
...
def edit_product_file(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_product_file, product_id)
...
def handle_new_product_file(message, product_id):
    ...
    con = pymysql.connect(
        host=config.MySQL[0], user=config.MySQL[1], passwd=config.MySQL[2], db=config.MySQL[3])
    cur = con.cursor()
    cur.execute("UPDATE market SET file_id = %s, file_name = %s WHERE id = %s",
                (new_file, new_file_name, product_id))
    con.commit()
    cur.close()
    con.close()
...
def edit_product_photo(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_product_photo, product_id)
...
def handle_new_product_photo(message, product_id):
    ...
    con = pymysql.connect(
        host=config.MySQL[0], user=config.MySQL[1], passwd=config.MySQL[2], db=config.MySQL[3])
    cur = con.cursor()
    cur.execute("UPDATE market SET photo = %s WHERE id = %s",
                (new_photo, product_id))
    con.commit()
    cur.close()
    con.close()
...
def edit_product_opisanie(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_product_opisanie, product_id)
...
def handle_new_product_opisanie(message, product_id):
    ...
    con = pymysql.connect(
        host=config.MySQL[0], user=config.MySQL[1], passwd=config.MySQL[2], db=config.MySQL[3])
    cur = con.cursor()
    cur.execute("UPDATE market SET opisanie = %s WHERE id = %s",
                (new_opisanie, product_id))
    con.commit()
    cur.close()
    con.close()
...
def edit_product_quantity(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_product_quantity, product_id)
...
def handle_new_product_quantity(message, product_id):
    ...
    con = pymysql.connect(
        host=config.MySQL[0], user=config.MySQL[1], passwd=config.MySQL[2], db=config.MySQL[3])
    cur = con.cursor()
    cur.execute("UPDATE market SET kolvo = %s WHERE id = %s",
...
def edit_product_price(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_product_price, product_id)
...
def handle_new_product_price(message, product_id):
    ...
    con = pymysql.connect(
        host=config.MySQL[0], user=config.MySQL[1], passwd=config.MySQL[2], db=config.MySQL[3])
    cur = con.cursor()
    cur.execute("UPDATE market SET price = %s WHERE id = %s",
...
def edit_product_name(call):
    ...
    bot.register_next_step_handler(
        call.message, handle_new_product_name, product_id)
...
def handle_new_product_name(message, product_id):
    ...
    con = pymysql.connect(
        host=config.MySQL[0], user=config.MySQL[1], passwd=config.MySQL[2], db=config.MySQL[3])
    cur = con.cursor()
    cur.execute("UPDATE market SET name_tovar = %s WHERE id = %s",
                (new_name, product_id))
    con.commit()
    cur.close()
    con.close()
...
def got_payment(message):
    core.update_balance(user_id=message.from_user.id,
...
def handle_give_balance_user_id(message):
    ...
    if core.profile_exists(user_id):
        ...
        bot.send_message(
            message.chat.id, 'Ошибка. Некорректный ID пользователя.')
        return

    bot.send_message(message.chat.id, 'Введите сумму для выдачи:')
    bot.register_next_step_handler(
        message, handle_give_balance_amount, user_id)
...
"""
    ...
    my_purchases_button = telebot.types.InlineKeyboardButton(
        text="Мои покупки", callback_data="my_purchases")
    keyboard.add(my_purchases_button)

    bot.send_message(message.chat.id, text=message_data,
...
def handle_my_purchases_callback(call):
    ...
    if purchased_items:
        ...
        for item in purchased_items:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text=item, callback_data=f"view_receipt_{item}"))

        bot.send_message(
            call.message.chat.id, "Выберите товар для просмотра чека:", reply_markup=keyboard)
    else:
        bot.send_message(call.message.chat.id,
                         "У вас нет совершенных покупок.")


@bot.callback_query_handler(func=lambda call: call.data.startswith("view_receipt_"))
def handle_view_receipt_callback(call):
    receipt_item = call.data.split("_")[2]
...
🕰 Дата покупки: <code>{str(buy_date)}</code>
        ...
        bot.send_message(call.message.chat.id,
                         text=message_check, parse_mode="HTML")
...
def delete_category(message):
    ...
    for category in categories:
        ...
        button = types.InlineKeyboardButton(
            text=category[1], callback_data=callback_data)
        keyboard.add(button)

    bot.send_message(chat_id=message.chat.id,
                     text="Выберите категорию, которую нужно удалить:", reply_markup=keyboard)
...
def delete_product(message):
    ...
    for category in categories:
        ...
        keyboard.add(button)
    bot.send_message(chat_id=message.chat.id,
                     text=categories_message, reply_markup=keyboard)
...
def category_selection(call):
    ...
    cur = con.cursor()
    cur.execute(
        f"SELECT id, name_tovar FROM market WHERE category_id={category_id}")
    products = cur.fetchall()
    if not products:
        bot.answer_callback_query(
            callback_query_id=call.id, text="В этой категории нет товаров.")
        return
    ...
    for product in products:
        ...
        button = types.InlineKeyboardButton(
            text=product[1], callback_data=callback_data)
        keyboard.add(button)
    ...
    bot.send_message(chat_id=call.message.chat.id,
                     text=products_message, reply_markup=keyboard)
    cur.close()
    con.close()
...
➖➖➖➖➖➖➖➖➖➖
    ...
    if categories:
        ...
        for category in categories:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text=category[1], callback_data=f"category_{category[0]}"))
...
➖➖➖➖➖➖➖➖➖➖
    ...
    if products:
        ...
        for product in products:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text=product[1], callback_data=f"product_{product[0]}_{category_id}"))
...
📃 Описание: <b>{product[6]}</b>
        ...
        if str(user_id) == config.ADMIN_ID:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text="Скачать файл", callback_data=f"open_file_{product_id}"))
        else:
            keyboard.add(telebot.types.InlineKeyboardButton(
                text="Купить товар", callback_data=f"buy_file_{product_id}"))
        chat_id = call.message.chat.id
        bot.send_photo(
            chat_id,  product[7], caption=message, reply_markup=keyboard, parse_mode="HTML")
...
def send_file_user(call):
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT file_id, file_name, price, kolvo, type, opisanie, name_tovar FROM market WHERE id=%s", product_id)
    result = cur.fetchone()
    ...
    if result:
        ...
        if float(balance_user) >= float(price):
            ...
            if kolvo > 0:
                kolvo -= 1
                cur.execute(
                    "UPDATE market SET kolvo = %s WHERE id = %s", (kolvo, product_id))
                con.commit()

                bot.send_document(call.message.chat.id,
                                  file_id, caption=file_name)
...
🕰 Дата покупки: <code> {str(buy_date)}</code>
            ...
            bot.send_message(
                call.message.chat.id, "У вас недостаточно средств для покупки этого товара.")
    else:
...
def process_category_name(message):
    ...
    bot.send_message(
        message.chat.id, f'Категория "{category_name}" успешно добавлена')
...
def add_item(message):
    ...
    for category in categories:
        button = types.InlineKeyboardButton(
            text=category[1], callback_data=f"add_item_{category[0]}")
        markups.add(button)
    bot.send_message(chat_id=message.chat.id,
                     text='Выберете категорию', reply_markup=markups)
...
def add_item_to_category(call):
    ...
    bot.register_next_step_handler(
        msg, lambda message: add_tovar_price(message, category_id))
...
def add_tovar_price(message, category_id):
    ...
    bot.register_next_step_handler(
        message, add_tovar_kolvo, name_tovar, category_id)
...
def add_tovar_kolvo(message, name_tovar, category_id):
    ...
    bot.register_next_step_handler(
        message, add_tovar_description, name_tovar, price, category_id)
...
def add_tovar_description(message, name_tovar, price, category_id):
    ...
    bot.register_next_step_handler(
        message, add_tovar_photo, name_tovar, price, category_id, kolvo)
...
def add_tovar_photo(message, name_tovar, price, category_id, kolvo):
    ...
    bot.register_next_step_handler(
        message, add_tovar_file, name_tovar, price, category_id, kolvo, opisanie)
...
def start(message):
    ...
    if not core.profile_exists(user_id):
        ...
        bot.send_message(message.chat.id, 'Ваш профиль успешно создан. Чтобы его посмотреть нажмите на кнопку 👤 Мой профиль',
...
def agent(message):
    ...
    if core.check_agent_status(user_id) == True:
        bot.send_message(message.chat.id, '🔑 Вы авторизованы как Агент поддержки',
                         parse_mode='html', reply_markup=markup.markup_agent())

    else:
        take_password_message = bot.send_message(
            message.chat.id, '⚠️ Тебя нет в базе. Отправь одноразовый пароль доступа.', reply_markup=markup.markup_cancel())

        bot.clear_step_handler_by_chat_id(message.chat.id)
        bot.register_next_step_handler(
            take_password_message, get_password_message)
...
def send_text(message):
    ...
    if message.text == '✏️ Написать запрос':
        take_new_request = bot.send_message(
            message.chat.id, 'Введите свой запрос и наши сотрудники скоро с вами свяжутся.', reply_markup=markup.markup_cancel())
        ...
        if value == 0:
            bot.send_message(
                message.chat.id, 'У вас пока ещё нет запросов.', reply_markup=markup.markup_main())
        else:
...
def get_password_message(message):
    ...
    if password == None:
        send_message = bot.send_message(
            message.chat.id, '⚠️ Вы отправляете не текст. Попробуйте еще раз.', reply_markup=markup.markup_cancel())
        ...
        bot.send_message(message.chat.id, '🔑 Вы авторизованы как Агент поддержки',
                         parse_mode='html', reply_markup=markup.markup_main())
        bot.send_message(message.chat.id, 'Выберите раздел технической панели:',
                         parse_mode='html', reply_markup=markup.markup_agent())

    else:
        send_message = bot.send_message(
            message.chat.id, '⚠️ Неверный пароль. Попробуй ещё раз.', reply_markup=markup.markup_cancel())
...
def get_agent_id_message(message):
    ...
    if agent_id == None:
        take_agent_id_message = bot.send_message(
            message.chat.id, '⚠️ Вы отправляете не текст. Попробуйте еще раз.', reply_markup=markup.markup_cancel())

        bot.clear_step_handler_by_chat_id(message.chat.id)
        bot.register_next_step_handler(
            take_agent_id_message, get_agent_id_message)
        ...
        bot.send_message(message.chat.id, '✅ Агент успешно добавлен.',
                         reply_markup=markup.markup_main())
        bot.send_message(message.chat.id, 'Выберите раздел админ панели:',
                         reply_markup=markup.markup_admin())
...
def get_new_request(message):
    ...
    if check_file != None:
        ...
        if str(request) == 'None':
            take_new_request = bot.send_message(
                message.chat.id, '⚠️ Вы не ввели ваш запрос. Попробуйте ещё раз, отправив текст вместе с файлом.', reply_markup=markup.markup_cancel())
            ...
            bot.send_message(
                message.chat.id, f'✅ Ваш запрос под ID {req_id} создан. Посмотреть текущие запросы можно нажав кнопку <b>Мои текущие запросы</b>', parse_mode='html', reply_markup=markup.markup_main())

    else:
        if request == None:
            take_new_request = bot.send_message(
                message.chat.id, '⚠️ Отправляемый вами тип данных не поддерживается в боте. Попробуйте еще раз отправить ваш запрос, использовав один из доступных типов данных (текст, файлы, фото, видео, аудио, голосовые сообщения)', reply_markup=markup.markup_cancel())
...
def get_additional_message(message, req_id, status):
    ...
    if additional_message == None:
        take_additional_message = bot.send_message(
            chat_id=message.chat.id, text='⚠️ Отправляемый вами тип данных не поддерживается в боте. Попробуйте еще раз отправить ваше сообщение, использовав один из доступных типов данных (текст, файлы, фото, видео, аудио, голосовые сообщения).', reply_markup=markup.markup_cancel())

        bot.clear_step_handler_by_chat_id(message.chat.id)
        bot.register_next_step_handler(
            take_additional_message, get_additional_message, req_id, status)
        ...
        if status == 'agent':
            ...
            try:
                ...
                if type == 'photo':
                    ...
                    bot.send_audio(user_id, audio=file_id,
                                   reply_markup=markup.markup_main())
                elif type == 'voice':
                    bot.send_voice(user_id, voice=file_id,
                                   reply_markup=markup.markup_main())
                else:
                    bot.send_message(user_id, additional_message,
                                     reply_markup=markup.markup_main())
            except:
                pass
...
def callback_inline(call):
    ...
    if call.message:
        if ('my_reqs:' in call.data) or ('waiting_reqs:' in call.data) or ('answered_reqs:' in call.data) or ('confirm_reqs:' in call.data):
            ...
            if value == 0:
                bot.send_message(chat_id=call.message.chat.id,
                                 text='⚠️ Запросы не обнаружены.', reply_markup=markup.markup_main())
                bot.answer_callback_query(call.id)
                return

            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='Нажмите на запрос, чтобы посмотреть историю переписки, либо добавить сообщение:', reply_markup=markup_req)
            except:
                bot.send_message(chat_id=call.message.chat.id,
                                 text='Ваши запросы:', reply_markup=markup_req)
            ...
            for data in request_data:
                if i == len_req_data:
                    markup_req = markup.markup_request_action(
                        req_id, req_status, callback)
                else:
                    markup_req = None

                bot.send_message(chat_id=call.message.chat.id, text=data,
                                 parse_mode='html', reply_markup=markup_req)
            ...
            take_additional_message = bot.send_message(
                chat_id=call.message.chat.id, text='Отправьте ваше сообщение, использовав один из доступных типов данных (текст, файлы, фото, видео, аудио, голосовые сообщения)', reply_markup=markup.markup_cancel())

            bot.register_next_step_handler(
                take_additional_message, get_additional_message, req_id, status_user)
            ...
            if core.get_req_status(req_id) == 'confirm':
                bot.send_message(chat_id=call.message.chat.id,
                                 text="⚠️ Этот запрос уже завершен.", reply_markup=markup.markup_main())
                bot.answer_callback_query(call.id)

                return

            if confirm_status == 'wait':
                bot.send_message(chat_id=call.message.chat.id, text="Для завершения запроса - нажмите кнопку <b>Подтвердить</b>",
                ...
                try:
                    ...
                    bot.send_message(
                        chat_id=call.message.chat.id, text="✅ Запрос успешно завершён.", reply_markup=markup.markup_main())
            ...
            if value == 0:
                bot.send_message(chat_id=call.message.chat.id,
                                 text='⚠️ Файлы не обнаружены.', reply_markup=markup.markup_main())
                bot.answer_callback_query(call.id)
                return
            ...
            try:
                if type == 'photo':
                    bot.send_photo(call.message.chat.id, photo=file_id,
                                   reply_markup=markup.markup_main())
                elif type == 'document':
                    bot.send_document(
                        call.message.chat.id, document=file_id, reply_markup=markup.markup_main())
                elif type == 'video':
                    bot.send_video(call.message.chat.id, video=file_id,
                                   reply_markup=markup.markup_main())
                elif type == 'audio':
                    bot.send_audio(call.message.chat.id, audio=file_id,
                                   reply_markup=markup.markup_main())
                elif type == 'voice':
                    bot.send_voice(call.message.chat.id, voice=file_id,
                                   reply_markup=markup.markup_main())

                bot.answer_callback_query(call.id)
            except:
                bot.send_message(call.message.chat.id,
                                 'Произошла ошибка', parse_mode='html')

        elif call.data == 'back_agent':
            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='🔑 Вы авторизованы как Агент поддержки', parse_mode='html', reply_markup=markup.markup_agent())
            except:
                bot.send_message(call.message.chat.id, '🔑 Вы авторизованы как Агент поддержки',
                                 parse_mode='html', reply_markup=markup.markup_agent())
            ...
            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='🔑 Вы авторизованы как Админ', parse_mode='html', reply_markup=markup.markup_admin())
            except:
                bot.send_message(call.message.chat.id, '🔑 Вы авторизованы как Админ',
                                 parse_mode='html', reply_markup=markup.markup_admin())

            bot.answer_callback_query(call.id)

        elif call.data == 'add_agent':
            take_agent_id_message = bot.send_message(
                chat_id=call.message.chat.id, text='Чтобы добавить агента поддержки - введите его ID Telegram.', reply_markup=markup.markup_cancel())
            bot.register_next_step_handler(
                take_agent_id_message, get_agent_id_message)
            ...
            if len_agents == 0:
                bot.send_message(chat_id=call.message.chat.id,
                                 text='⚠️ Агенты не обнаружены.', reply_markup=markup.markup_main())
                bot.answer_callback_query(call.id)
                return

            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='Нажмите на агента поддержки, чтобы удалить его', parse_mode='html', reply_markup=markup_agents)
            except:
            ...
            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='Нажмите на агента поддержки, чтобы удалить его', parse_mode='html', reply_markup=markup.markup_agents('1')[0])
            except:
                bot.send_message(call.message.chat.id, 'Нажмите на агента поддержки, чтобы удалить его',
            ...
            if len_passwords == 0:
                bot.send_message(chat_id=call.message.chat.id,
                                 text='⚠️ Пароли не обнаружены.', reply_markup=markup.markup_main())
                bot.answer_callback_query(call.id)
                return

            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='Нажмите на пароль, чтобы удалить его', parse_mode='html', reply_markup=markup_passwords)
            except:
                bot.send_message(call.message.chat.id, 'Нажмите на пароль, чтобы удалить его',
                                 parse_mode='html', reply_markup=markup_passwords)
            ...
            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='Нажмите на пароль, чтобы удалить его', parse_mode='html', reply_markup=markup.markup_passwords('1')[0])
            except:
                bot.send_message(call.message.chat.id, 'Нажмите на пароль, чтобы удалить его',
                                 parse_mode='html', reply_markup=markup.markup_passwords('1')[0])
            ...
            if status == 'wait':
                try:
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                          text=f"Вы точно хотите отключить бота?", parse_mode='html', reply_markup=markup.markup_confirm_stop())
                except:
                    bot.send_message(call.message.chat.id, f"Вы точно хотите отключить бота?",
                                     parse_mode='html', reply_markup=markup.markup_confirm_stop())

            elif status == 'confirm':
                try:
                    bot.edit_message_text(
                        chat_id=call.message.chat.id, message_id=call.message.message_id, text='✅ Бот отключен.')
                except:
                    bot.send_message(
                        chat_id=call.message.chat.id, text='✅ Бот отключен.')
            ...
            for item in items:
                ...
                button = types.InlineKeyboardButton(
                    text=item[1], callback_data=f"view_item_{item[0]}")
                markups.add(button)
            bot.edit_message_text(chat_id=call.message.chat.id, text='Выберете товар',
                                  reply_markup=markups, message_id=call.message.id)

        elif call.data.startswith('give_backup'):
            market_admin.get_backup(call.message)
        elif call.data.startswith('market_admin'):
            bot.send_message(call.message.chat.id, f"Выберете действие",
                             reply_markup=markup.markup_tovars_admin())
